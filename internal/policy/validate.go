...
func Validate(p *policyv1.Policy) error {
	...
	default:
		return fmt.Errorf("[ERR-357] unknown policy type %T", pt)
	}
}
...
func validateResourcePolicy(rp *policyv1.ResourcePolicy) (err error) {
	...
	for i, rule := range rp.Rules {
		...
		if len(rule.Roles) == 0 && len(rule.DerivedRoles) == 0 {
			err = multierr.Append(err, fmt.Errorf("[ERR-358] rule %s does not specify any roles or derived roles to match", ruleName))
		}
		...
		if idx, exists := ruleNames[rule.Name]; exists {
			err = multierr.Append(err, fmt.Errorf("[ERR-359] rule #%d has the same name as rule #%d: '%s'", i+1, idx, rule.Name))
		} else {
	...
	for i, resourceRules := range rp.Rules {
		if idx, exists := resourceNames[resourceRules.Resource]; exists {
			err = multierr.Append(err,
				fmt.Errorf("[ERR-360] duplicate definition of resource '%s' at #%d (previous definition at #%d)", resourceRules.Resource, i+1, idx))
		} else {
		...
		for j, actionRule := range resourceRules.Actions {
			...
			if idx, exists := ruleNames[actionRule.Name]; exists {
				err = multierr.Append(err,
					fmt.Errorf("[ERR-361] action rule #%d for resource %s has the same name as action rule #%d: '%s'",
...
func validateDerivedRoles(dr *policyv1.DerivedRoles) (err error) {
	...
	for i, rd := range dr.Definitions {
		...
		if idx, exists := roleNames[rd.Name]; exists {
			err = multierr.Append(err, fmt.Errorf("[ERR-362] derived role definition #%d has the same name as definition #%d: '%s'", i+1, idx, rd.Name))
		} else {
		...
		if len(rd.ParentRoles) == 0 {
			err = multierr.Append(err, fmt.Errorf("[ERR-363] derived role definition '%s' does not specify any parent roles to match", rd.Name))
		}
	}
